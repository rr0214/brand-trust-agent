...
if run_btn and user_prompt.strip():
    ...
    if num_campaigns > 1:
        ...
        for i in range(num_campaigns):
            ...
            with _tracer.start_as_current_span(f"campaign-{i+1}") as campaign_span:
                campaign_span.set_attribute(SpanAttributes.OPENINFERENCE_SPAN_KIND, "CHAIN")
                campaign_span.set_attribute(SpanAttributes.INPUT_VALUE, user_prompt)
                ...
                # Capture OTel context here so child spans stay attached to this
                # campaign's CHAIN span even across Streamlit st.status() boundaries,
                # which can break Python contextvars propagation.
                from opentelemetry import context as otel_context
                campaign_ctx = otel_context.get_current()

                with st.status("🔍 Researching brand facts...", expanded=True) as s1:
                    research_query = f"What brand facts, certifications, and approved claims support this campaign: {user_prompt}"
                    _tok1 = otel_context.attach(campaign_ctx)
                    try:
                        research = run_brand_research(
                            query=research_query,
                            include_poisoned=config["include_poisoned"],
                ...
                with st.status("📣 Building campaign strategy...", expanded=True) as s2:
                    _tok2 = otel_context.attach(campaign_ctx)
                    try:
                        strategy = run_campaign_strategy(
                            research=research,
                            campaign_brief=user_prompt,
                ...
                with st.status("🎬 Generating creative...", expanded=True) as s3:
                    _tok3 = otel_context.attach(campaign_ctx)
                    try:
                        creative = run_creative_execution(strategy=strategy, brand_name="Verdant")
                    finally:
                        otel_context.detach(_tok3)
                    t3 = time.time()
        ...
        # Force-flush all series spans to Arize
        from instrumentation.arize_setup import flush_traces
        flush_traces()
    ...
    # ── SINGLE CAMPAIGN MODE ──────────────────────────────────────────────
        ...
        # Force-flush spans to Arize before rendering results
        from instrumentation.arize_setup import flush_traces
        flush_traces()
